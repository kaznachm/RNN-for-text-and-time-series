...
def categorical_crossentropy_3d(coding_dist, true_dist, lengths=None):
    ...
    """#OLD ONE (28.05) - problem was in that cross entropy was negative because I did summation over the row and took -log after, but first I need to take logs and then (-sum) them
	# Zero out the false probabilities and sum the remaining true probabilities to remove the third dimension.
	indexes = theano.tensor.arange(coding_dist.shape[2])
	mask = theano.tensor.neq(indexes, true_dist.reshape((true_dist.shape[0], true_dist.shape[1], 1)))
	pred_probs = theano.tensor.set_subtensor(coding_dist[theano.tensor.nonzero(mask)], 0.).sum(axis=2)
...
def crossentropy_3d(coding_dist, true_dist):
	pred_probs = T.log(coding_dist) * true_dist
	pred_probs_per_sample = pred_probs.sum(axis=2).sum(axis=1)
	return -pred_probs_per_sample#-theano.tensor.log(pred_probs_per_sample)


def get_metric_probs(coding_dist, true_dist, lengths=None):
    #http://stackoverflow.com/questions/30225633/cross-entropy-for-batch-with-theano
    ...
    mask = theano.tensor.neq(indexes, true_dist.reshape((true_dist.shape[0], true_dist.shape[1], 1)))
    pred_probs = theano.tensor.set_subtensor(coding_dist[theano.tensor.nonzero(mask)], 0.).sum(axis=2)
    pred_probs_log = theano.tensor.log(pred_probs)
    pred_probs_per_sample = pred_probs_log.sum(axis=1)#pred_probs_per_sample = pred_probs_log.prod(axis=1) #here is the difference with categorical_crossentropy_3d()
    return pred_probs_per_sample #theano.tensor.log(pred_probs_per_sample)
...
"""
def batch_gen_text(X, X2, y_wv, y_w, batch_size): #(X, y, X_mask, X2, X_mask2, X3, X_mask3, X4, X_mask4, batch_size): #copy of batch_gen_mvrt4()
    
    #randomly samples intances for the given mini-batch size
        
    while True:
        idx = np.random.choice(len(y_wv), batch_size)
        yield X[idx,:].astype('float32'), X2[idx,:].astype('float32'), y_wv[idx,:,:].astype('float32'), y_w[idx,:].astype('int64') #yield X[idx,:].astype('float32'), y[idx,:].astype('float32'), X_mask[idx,:].astype('float32'), X2[idx,:].astype('float32'), X_mask2[idx,:].astype('float32'), X3[idx,:].astype('float32'), X_mask3[idx,:].astype('float32'), X4[idx,:].astype('float32'), X_mask4[idx,:].astype('float32')
"""
def batch_gen_text(X, y_wv, y_w, batch_size): #(X, y, X_mask, X2, X_mask2, X3, X_mask3, X4, X_mask4, batch_size): #copy of batch_gen_mvrt4()
    """
    randomly samples intances for the given mini-batch size
    """
    while True:
        idx = np.random.choice(len(y_wv), batch_size)
        yield X[idx].astype('float64'), y_wv[idx,:,:].astype('float64'), y_w[idx,:].astype('int64') #yield X[idx,:].astype('float64'), y_wv[idx,:,:].astype('float64'), y_w[idx,:].astype('int64') ##yield X[idx,:].astype('float32'), X2[idx,:].astype('float32'), y_wv[idx,:,:].astype('float32'), y_w[idx,:].astype('int64') #yield X[idx,:].astype('float32'), y[idx,:].astype('float32'), X_mask[idx,:].astype('float32'), X2[idx,:].astype('float32'), X_mask2[idx,:].astype('float32'), X3[idx,:].astype('float32'), X_mask3[idx,:].astype('float32'), X4[idx,:].astype('float32'), X_mask4[idx,:].astype('float32')
...
def train_test_X_y_mask(params):
    ...
    if params['data_name'] == 'coservit': #####
        data = pickle.load(open(params['data_file'], 'rb')) #####
...
def compute_X_y_text(data, text_wv, text_w, params): #no mask!!!!! #copy of compute_X_y_mask()
    #preparing data
    train_data, test_data = get_train_test(data, params['train_test_ratio'])
    train_data, val_data = get_train_test(train_data, params['train_val_ratio'])
    #print(data.shape) #(1383, 288, 2)
    
    train_wv, test_wv = get_train_test(text_wv, params['train_test_ratio']) #word vectors
    train_wv, val_wv = get_train_test(train_wv, params['train_val_ratio'])
    
    train_w, test_w = get_train_test(text_w, params['train_test_ratio']) #word 1hots, indexes not whole 1hot vector of length |vocab_size|
    train_w, val_w = get_train_test(train_w, params['train_val_ratio'])
    
    X_train, y_train_wv, y_train_w, X_val, y_val_wv, y_val_w, X_test, y_test_wv, y_test_w = train_data, train_wv, train_w, val_data, val_wv, val_w, test_data, test_wv, test_w 
    
    return (X_train, y_train_wv, y_train_w, X_val, y_val_wv, y_val_w, X_test, y_test_wv, y_test_w)
    #return (X_train[tr,:], y_train[tr,:], X_train_mask[tr,:], X_val[vl,:], y_val[vl,:], X_val_mask[vl,:], X_test[tt,:], y_test[tt,:], X_test_mask[tt,:])
...
def train_test_X_y_text(params): #no mask!!!!! #copy of train_test_X_y_mask()
    """
    Creates X and y matrices for train, validation and test sets and 
    #corresponding mask which indicates missing values of X
    """
    if params['data_name'] == 'coservit': #####
        data = pickle.load(open(params['data_file'], 'rb')) #####
        text_wv = np.load(params['text_file_wv']) #pickle.load(open(params['text_file'], 'rb'))
        text_w = np.load(params['text_file_w']) ##
        #print(type(data), data.shape)
        #print(type(text))
        X_train, y_train_wv, y_train_w, X_val, y_val_wv, y_val_w, X_test, y_test_wv, y_test_w = compute_X_y_text(data, text_wv, text_w, params) ##### 'X' should be slices later as it is many metrics
    else:
        print('rewrite <train_test_X_y_mask_text()> for not coservit data')
        
    #print(X_train.shape, y_train_wv.shape, y_train_w.shape, X_val.shape, y_val_wv.shape, y_val_w.shape, X_test.shape, y_test_wv.shape, y_test_w.shape) #((777, 288, 2), (777, 27, 200), (777, 27), (260, 288, 2), (260, 27, 200), (260, 27), (346, 288, 2), (346, 27, 200), (346, 27))
    return X_train, y_train_wv, y_train_w, X_val, y_val_wv, y_val_w, X_test, y_test_wv, y_test_w
...
def network_mvrt4(params):
    ...
    if params['attention']:
        ...
        if params['num_layers'] ==1:
            ...
            l_out, l_out_val= model_seq2seq_mvrt4_GRU(X_enc_sym, mask_enc, X_enc_sym2, mask_enc2, X_enc_sym3, mask_enc3, X_enc_sym4, mask_enc4, X_dec_sym, mask_dec, params['windowsize'], params['horizon'],  hidden_size = params['num_units'], 
                                                            grad_clip = params['grad_clipping'])
...
def network_text(params): # copy of network_mvrt4()
    """
    builds the network according to given optimization and model parameters and data
    """
    X_enc_sym = T.dtensor3('x_enc_sym') ##X_enc_sym = T.ftensor3('x_enc_sym')
    ##X_enc_sym2 = T.ftensor3('x_enc_sym2')
    ...
    if params['attention']:
        ...
        network_output, network_output_val = lasagne.layers.get_output([l_out, l_out_val]) #network_output, network_output_val, alpha_weights = lasagne.layers.get_output([l_out, l_out_val, alphas])
    else:
        if params['num_layers'] ==1:
            #l_out, l_out_val= model_seq2seq_mvrt4(X_enc_sym, mask_enc, X_enc_sym2, mask_enc2, X_enc_sym3, mask_enc3, X_enc_sym4, mask_enc4, X_dec_sym, mask_dec, params['windowsize'], params['horizon'],  hidden_size = params['num_units'], 
                                                #grad_clip = params['grad_clipping'])
            l_out, l_out_val = model_seq2seq_GRU_text(X_enc_sym, X_dec_sym, Emb_mtx_sym, params['windowise'], params['horizon']-1,  hidden_size = params['num_units'], 
                                                                        grad_clip = params['grad_clipping'], vocab_size = params['vocab_size'], word_dim = params['word_dim']) ##l_out, l_out_val = model_seq2seq_GRU_text(X_enc_sym, X_enc_sym2, X_dec_sym, params['windowise'], params['horizon']-1,  hidden_size = params['num_units'], 
                                                            ##grad_clip = params['grad_clipping'], vocab_size = params['vocab_size'], word_dim = params['word_dim']) #l_out, l_out_val = model_seq2seq_GRU_text(X_enc_sym, X_enc_sym2, X_dec_sym, params['windowise'], params['horizon'],  hidden_size = params['num_units'], 
                                                            #grad_clip = params['grad_clipping'], vocab_size = params['vocab_size'], word_dim = params['word_dim']) #l_out, l_out_val= model_seq2seq_mvrt4_GRU(X_enc_sym, mask_enc, X_enc_sym2, mask_enc2, X_enc_sym3, mask_enc3, X_enc_sym4, mask_enc4, X_dec_sym, mask_dec, params['windowsize'], params['horizon'],  hidden_size = params['num_units'], 
                                                            #grad_clip = params['grad_clipping'])
        network_output, network_output_val = lasagne.layers.get_output([l_out, l_out_val])
        #network_output, network_output_val = lasagne.layers.get_output([l_out, l_out_val]) # why I have the second one, wtf... 
        #
    weights = lasagne.layers.get_all_params(l_out,trainable=True)
    if params['regularization_type'] == 'l1':
        reg_loss = lasagne.regularization.regularize_network_params(l_out, l1) * params['lambda_regularization']
    ...
    #loss_T = T.nnet.categorical_crossentropy(network_output, y_sym).mean() + reg_loss #loss_T = T.mean(lasagne.objectives.squared_error(network_output, y_sym)) + reg_loss
    #loss_val_T = T.nnet.categorical_crossentropy(network_output_val, y_sym).mean() 
    ...
    if params['attention'] and (params['att_type']=='lambda_mu' or params['att_type']=='lambda_mu_alt'):
        ...
        f_train = theano.function([X_enc_sym, X_dec_sym, y_sym], loss_T, updates=updates, allow_input_downcast=True) ##f_train = theano.function([X_enc_sym, X_enc_sym2, X_dec_sym, y_sym], loss_T, updates=updates, allow_input_downcast=True) #f_train = theano.function([X_enc_sym, mask_enc, X_enc_sym2, mask_enc2, X_enc_sym3, mask_enc3, X_enc_sym4, mask_enc4, X_dec_sym, mask_dec, y_sym], loss_T, updates=updates, allow_input_downcast=True)
        f_val = theano.function([X_enc_sym, X_dec_sym, Emb_mtx_sym, y_sym], loss_val_T, allow_input_downcast=True, on_unused_input='ignore') ##f_val = theano.function([X_enc_sym, X_enc_sym2, X_dec_sym, y_sym], loss_val_T, allow_input_downcast=True) #f_val = theano.function([X_enc_sym, mask_enc, X_enc_sym2, mask_enc2, X_enc_sym3, mask_enc3, X_enc_sym4, mask_enc4, X_dec_sym, mask_dec, y_sym], loss_val_T, allow_input_downcast=True)
    if params['attention']:  
        if params['att_type']=='lambda_mu' or params['att_type']=='lambda_mu_alt':
            if params['interpolation'] and params['att_type']=='lambda_mu':
                forecast = theano.function([X_enc_sym, mask_enc, X_enc_sym2, mask_enc2, X_enc_sym3, mask_enc3, X_enc_sym4, mask_enc4, X_dec_sym, mask_dec,
            ...
            forecast = theano.function([X_enc_sym, X_dec_sym, Emb_mtx_sym, y_sym], loss_test, allow_input_downcast=True)
        else:
            forecast = theano.function([X_enc_sym, mask_enc, X_enc_sym2, mask_enc2, X_enc_sym3, mask_enc3, X_enc_sym4, mask_enc4, X_dec_sym, mask_dec], [network_output_val, alpha_weights], allow_input_downcast=True)
    else:
        forecast = theano.function([X_enc_sym, X_dec_sym, Emb_mtx_sym, y_sym], loss_test, allow_input_downcast=True, on_unused_input='ignore') ##forecast = theano.function([X_enc_sym, X_enc_sym2, X_dec_sym], network_output_val, allow_input_downcast=True) #forecast = theano.function([X_enc_sym, mask_enc, X_enc_sym2, mask_enc2, X_enc_sym3, mask_enc3, X_enc_sym4, mask_enc4, X_dec_sym, mask_dec], network_output_val, allow_input_downcast=True)
        f_metric_probs = theano.function([X_enc_sym, X_dec_sym, y_sym], metric_probs, allow_input_downcast=True) #####
    return l_out, f_train, f_val, forecast, f_metric_probs #####
...
def train_and_test_text(params, results_directory): #copy of train_and_test_mvrt4()
    """
    builds the network and trains the model with according to the given parameters
    """
    logging.info('TRAINING:')
    logging.info('Loading data and creating regression matrix for time series')
    X_train, y_train_wv, y_train_w, X_val, y_val_wv, y_val_w, X_test, y_test_wv, y_test_w = train_test_X_y_text(params) #no mask!!! #X_train, y_train, X_train_mask, X_val, y_val, X_val_mask, X_test, y_test, X_test_mask = train_test_X_y_mask(params) #((777, 288, 2), (777, 27, 200), (777, 27), (260, 288, 2), (260, 27, 200), (260, 27), (346, 288, 2), (346, 27, 200), (346, 27))
    Emb_mtx = np.load(params['emb_file']) #np.array([[[1,2,3], [4,5,6], [0,2,1], [0,1,2]], [[6,7,8], [3,6,0], [4,3,2], [1,1,2]]])
    print(X_train.shape, y_train_wv.shape, y_train_w.shape, X_val.shape, y_val_wv.shape, y_val_w.shape, X_test.shape, y_test_wv.shape, y_test_w.shape) #((2333, 288), (2333, 27, 200), (2333, 27), (778, 288), (778, 27, 200), (778, 27), (1038, 288), (1038, 27, 200), (1038, 27))
    ##X_train2, X_val2, X_test2 = X_train[:,:,1], X_val[:,:,1], X_test[:,:,1] #metric 1
    ##X_train, X_val, X_test = X_train[:,:,0], X_val[:,:,0], X_test[:,:,0] #metric 0
    ...
    logging.info('Data is loaded')
    logging.info('Building network')
    l_out, f_train, f_val, forecast, f_metric_probs = network_text(params) ##### l_out, f_train, f_val, forecast = network_text(params)
    logging.info('Network is built')
    num_batches = len(X_train) // params['batch_size']
    #num_val_batches = len(X_val) // params['batch_size']
    # Minibatch generators for the training and validation sets
    train_batches = batch_gen_text(X_train, y_train_wv, y_train_w, params['batch_size']) ##train_batches = batch_gen_text(X_train, X_train2, y_train_wv, y_train_w, params['batch_size']) #train_batches = batch_gen_mvrt4(X_train, y_train, X_train_mask, X_train2, X_train_mask2, X_train3, X_train_mask3, X_train4, X_train_mask4, params['batch_size'])
    #val_batches = batch_gen_mvrt(X_val, y_val, X_val_mask, params['batch_size'], params) #1
    #batch_ones = np.ones((params['batch_size'],1,params['word_dim'])) # ????????? ИЛИ batch_ones = np.ones((params['batch_size'],params['vocab_size'])) #ИНИЦИАЛИЗАЦИЯ ПЕРВОЙ КОЛОНКИ ДЕКОДЕРА #batch_ones = np.ones((params['batch_size'],1)) # as a start signal 
    #X_mask_dec = np.ones((params['batch_size'], params['horizon']))
    #2:
    X_val = np.expand_dims(X_val, axis = 2) 
    X_val_dec = np.ones((np.size(y_val_wv,0), np.size(y_val_wv,1), np.size(y_val_wv,2)))  ################ #X_val_dec = np.ones((np.size(y_val,0), np.size(y_val,1)))
    #X_val_mask_dec = np.ones((np.size(y_val,0), np.size(y_val,1)))
    #X_val_dec = np.expand_dims(X_val_dec, axis = 2)  # no need because we are already 3d
    #multi-vrt
    #X_val2 = np.expand_dims(X_val2, axis = 2)
    #X_val3 = np.expand_dims(X_val3, axis = 2)
    #X_val4 = np.expand_dims(X_val4, axis = 2)
    ...
    while (epoch < params['max_num_epochs']) and (not done_looping):
        ...
        tr_loss = 0
        for batch_ind in range(num_batches):
            X, y_wv, y_w = next(train_batches) ##X, X2, y_wv, y_w = next(train_batches)#X, y, X_mask, X2, X_mask2, X3, X_mask3, X4, X_mask4 = next(train_batches)
            X = np.expand_dims(X, axis = 2) ##X, X2 = np.expand_dims(X, axis = 2), np.expand_dims(X2, axis = 2) #X, X2, X3, X4 = np.expand_dims(X, axis = 2), np.expand_dims(X2, axis = 2), np.expand_dims(X3, axis = 2), np.expand_dims(X4, axis = 2)
            X_dec = y_wv[:,:-1,:] #X_dec = np.hstack((batch_ones, y_wv[:,:-1,:])) #all elements except last one #X_dec = np.hstack((batch_ones, y[:,:-1]))
             #X_dec = np.expand_dims(X_dec, axis = 2)
            if params['attention'] and params['att_type']=='lambda_mu':
                d_inds, d_inds2, d_inds3, d_inds4 = delta_gap_mask(X_mask), delta_gap_mask(X_mask2), delta_gap_mask(X_mask3), delta_gap_mask(X_mask4)
                if params['interpolation']: 
                ...
                #print(X.shape, X2.shape, X_dec.shape, y_w.shape, y_w[:5, :5]) #(64, 288, 1), (64, 288, 1), (64, 27, 200), (64, 27), array([[155562,  66140,  55884,  52273,  58068].....
                loss = f_train(X, X_dec, y_w[:, 1:]) ##loss = f_train(X, X2, X_dec, y_w[:, 1:]) #loss = f_train(X, X_mask, X2, X_mask2, X3, X_mask3, X4, X_mask4, X_dec, X_mask_dec, y)
            iter = (epoch - 1) * num_batches + batch_ind # iteration number
            tr_loss += loss
        tr_loss /= (1.0*num_batches)
        loss_train.append(tr_loss)
        #2:
        if params['attention'] and params['att_type']=='lambda_mu':
            d_inds, d_inds2, d_inds3, d_inds4 = delta_gap_mask(X_val_mask), delta_gap_mask(X_val_mask2), delta_gap_mask(X_val_mask3), delta_gap_mask(X_val_mask4)
            ...
            #print(X_val.shape, X_val2.shape, X_val_dec.shape, y_val_w.shape) #((260, 288, 1), (260, 288, 1), (260, 27, 200), (260, 27))
            val_loss = f_val(X_val, X_val_dec, Emb_mtx, y_val_w[:, 1:]) ##val_loss = f_val(X_val, X_val2, X_val_dec, y_val_w[:, 1:]) #val_loss = f_val(X_val, X_val_mask, X_val2, X_val_mask2, X_val3, X_val_mask3, X_val4, X_val_mask4, X_val_dec, X_val_mask_dec, y_val)
        #end of 2:
        loss_validation.append(val_loss.mean()) ############# WHY THIS IS AN ARRAY WITH ONE NUMBER? IT SHOULD BE JUST A NUMBER LIKE IN TRAIN
        #print(val_loss, type(epoch), type(tr_loss), type(val_loss), val_loss.shape) #(array(6.923833615520672), <type 'int'>, <type 'numpy.float64'>, <type 'numpy.ndarray'>, ())
        logging.info('Epoch {}, Train (val) loss {:.03f} ({:.03f}) ratio {:.03f}'.format(
                epoch, tr_loss, val_loss.mean(), val_loss/tr_loss)) #epoch, tr_loss, val_loss.mean(), val_loss/tr_loss))
        if val_loss < best_validation_loss: # if we got the best validation score until now
            if (val_loss < best_validation_loss *improvement_threshold): #improve patience if loss improvement is good enough
                patience = max(patience, iter * params['patience_increase'])
            best_validation_loss = val_loss
    ...
    #
    X_test = np.expand_dims(X_test, axis = 2)
    X_dec_test = np.ones((np.size(y_test_wv,0), np.size(y_test_wv,1), np.size(y_test_wv,2))) #X_dec_test = np.ones((np.size(y_test,0), np.size(y_test,1)))
    #X_test_mask_dec =  np.ones((np.size(y_test,0), np.size(y_test,1)))
    #X_dec_test = np.expand_dims(X_dec_test, axis = 2)
    ##X_test2 = np.expand_dims(X_test2, axis = 2)
    #X_test3 = np.expand_dims(X_test3, axis = 2)
    ...
    if params['attention']:
        if params['att_type']=='lambda_mu':
            ...
            y_pred, y_weights = forecast(X_test, X_dec_test, Emb_mtx, y_test_w[:, 1:]) #----
        pickle.dump(y_weights, open(save_to + 'y_weights.pkl', 'wb'), protocol=pickle.HIGHEST_PROTOCOL)
        #pickle.dump(X_test, open(save_to + 'X_test.pkl', 'wb'), protocol=pickle.HIGHEST_PROTOCOL)
        #pickle.dump(X_test_mask, open(save_to + 'X_test_mask.pkl', 'wb'), protocol=pickle.HIGHEST_PROTOCOL)
    else:
        y_pred = forecast(X_test, X_dec_test, Emb_mtx, y_test_w[:, 1:]) ##y_pred = forecast(X_test, X_test2, X_dec_test) #y_pred = forecast(X_test, X_test_mask, X_test2, X_test_mask2, X_test3, X_test_mask3, X_test4, X_test_mask4, X_dec_test, X_test_mask_dec)
    #print(X_test.shape, X_test2.shape, X_dec_test.shape, type(y_pred), y_pred)   ###########
        m_probs = f_metric_probs(X_test, X_dec_test, y_test_w[:, 1:]) ##### 
        #print(m_probs) #####
    #print(type(y_pred)) #<type 'numpy.ndarray'>
    entropy = y_pred.mean() ##entropy = categorical_crossentropy_3d(y_pred, y_test_w[:, 1:]).mean() #mse = ((y_pred - y_test) ** 2).mean(axis=0)
    pickle.dump(entropy, open(save_to + 'entropy.pkl', 'wb'), protocol=pickle.HIGHEST_PROTOCOL) #pickle.dump(mse, open(save_to + 'mse.pkl', 'wb'), protocol=pickle.HIGHEST_PROTOCOL)
    logging.info('Test Prediction Entropy: ' + str(entropy))  #logging.info('Test Prediction Entropy: ' + str(entropy.mean()))
    pickle.dump(y_test_w, open(save_to + 'y_test_w.pkl', 'wb'), protocol=pickle.HIGHEST_PROTOCOL)
    pickle.dump(y_pred, open(save_to + 'y_pred.pkl', 'wb'), protocol=pickle.HIGHEST_PROTOCOL)
    #mape = mean_abs_percent_err(y_pred, y_test)
    #smape = sym_mean_abs_percent_err(y_pred, y_test)
    #logging.info('Test Prediction MAPE: ' + str(mape.mean()))
...
def main():
    ...
    results_directory = 'Results_multivariate_text_set/'
    #    
    train_and_test = True
    if len(sys.argv) > 5:
    ...
    logging.info(line_inds)
    for i in line_inds:
        params = load_parameters2_mvrt_text(sys.argv[1], i)
        ...
        if train_and_test:
            ...
            pass
            #test_mvrt4_text(params, results_directory) #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        #
        #test_mvrt4(params, results_directory)
        #        
        f = open(str(sys.argv[1])+"_mvrt_intr_GRU_text.log","a") #f = open(str(sys.argv[1])+"_mvrt_intr.log","a")
        f.write(str(i)+':')
        #f.write(line)
        f.close()
