...
import os
import linecache
from models import model_seq2seq_att_adist, model_seq2seq, model_seq2seq_att, model_seq2seq_att_lambda_mu, model_seq2seq_att_lambda, model_seq2seq_att_lambda_mu_alt
...
def compute_X_y_mask(data, data_mask, params):
    """
    returns input, target pairs X,y for train, validation and test 
    """
    #preparing data
    train, test = get_train_test(data, params['train_test_ratio']) # divides time series into two train and test sequence
    train, val = get_train_test(train, params['train_val_ratio']) #divides the train into two: train and validation
    ntrain_set, train_mean, train_std = normalize(train) # normalize train
    nval_set = normalize(val, mean = train_mean, std = train_std) # normalize val
    ntest_set = normalize(test, mean = train_mean, std = train_std) # normalize test
    ntrain_set, nval_set, ntest_set = replace_none_w_one(ntrain_set), replace_none_w_one(nval_set), replace_none_w_one(ntest_set)
    #creating X input and y target according to the windowsize-local history and horizon - dense
    ...
    #discarding the missing valued targets y
    ...
    #discarding whole missing sequence in input X
        ...
        if params['num_layers'] ==1:
            if params['att_type']=='original':
                ...
                l_out, l_out_val, alphas = model_seq2seq_att_lambda(X_enc_sym, mask_enc, X_dec_sym, mask_dec, params['windowsize'], params['horizon'],  hidden_size = params['num_units'], 
                                            grad_clip = params['grad_clipping'], att_size = params['num_att_units']) 
            elif params['att_type']=='lambda_mu': #tau-mu-2
                ...
                l_out, l_out_val, alphas = model_seq2seq_att_lambda_mu(X_enc_sym, mask_enc, X_dec_sym, mask_dec, delta_inds_sym, params['windowsize'], params['horizon'],  hidden_size = params['num_units'], 
                                            grad_clip = params['grad_clipping'], att_size = params['num_att_units'])
            elif params['att_type']=='lambda_mu_alt': #tau-mu-1
...
def train(params, results_directory):
    ...
    while (epoch < params['max_num_epochs']) and (not done_looping):
        ...
        for batch_ind in range(num_batches):
            ...
            tr_loss += loss #accumulating the loss
        tr_loss /= num_batches #averaging
...
def load_parameters2(params_csvfile, line_num): 
    ...
    params={}
    values = linecache.getline(params_csvfile,line_num)[:-1].split(',')
    params['num_units'] = int(values[0]) # number of hidden units
    params['num_att_units'] = int(values[1]) # number of attention units
    data_type = str(values[2]) #original, missing percentage -- orig/5/10/15/20
    params['data_name'] = str(values[3]) #data name
    model = str(values[4]) 
    ...
    if model == 'RNN':
        ...
        if model == 'RNN-ATT':
            ...
            #params['att_type'] = 'lambda_mu_alt' #for now needs to be changed!
...
def main():
    ...
    for i in line_inds:
        ...
        #        
        f = open(str(sys.argv[1])+"_alt_test.log","a")
        f.write(str(i)+':')
        #f.write(line)
        f.close()
...
"""    
    ...
    params['windowsize'] = 100 #local history - T
    params['horizon'] = 4 #forecast horizon - T'
