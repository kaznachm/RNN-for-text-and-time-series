...
from models_mvrt import model_seq2seq_mvrt4, model_seq2seq_att_mvrt4, model_seq2seq_att_lambda_mvrt4#, model_seq2seq_att_lambda_mu_mvrt4, model_seq2seq_att_lambda_mu_alt_mvrt4
import pandas as pd
...
def network_mvrt4(params):
    ...
    if params['attention']:
        if params['num_layers'] ==1:
            if params['att_type']=='original':
                ...
                l_out, l_out_val, alphas = model_seq2seq_att_lambda_mu_mvrt4(X_enc_sym, mask_enc, X_enc_sym2, mask_enc2, X_enc_sym3, mask_enc3, X_enc_sym4, mask_enc4, X_dec_sym, mask_dec, delta_inds_sym, params['windowsize'], params['horizon'],  hidden_size = params['num_units'], 
                                            grad_clip = params['grad_clipping'], att_size = params['num_att_units'])
            elif params['att_type']=='lambda_mu_alt':
                delta_inds_sym = T.matrix('d_inds') 
                l_out, l_out_val, alphas = model_seq2seq_att_lambda_mu_alt_mvrt4(X_enc_sym, mask_enc, X_dec_sym, mask_dec, delta_inds_sym, params['windowsize'], params['horizon'],  hidden_size = params['num_units'], 
                                            grad_clip = params['grad_clipping'], att_size = params['num_att_units'])
    ...
    if params['attention'] and (params['att_type']=='lambda_mu' or params['att_type']=='lambda_mu_alt'):
       f_train = theano.function([X_enc_sym, mask_enc, X_enc_sym2, mask_enc2, X_enc_sym3, mask_enc3, X_enc_sym4, mask_enc4, X_dec_sym, mask_dec, y_sym, delta_inds_sym], loss_T, updates=updates, allow_input_downcast=True)
       f_val = theano.function([X_enc_sym, mask_enc, X_enc_sym2, mask_enc2, X_enc_sym3, mask_enc3, X_enc_sym4, mask_enc4, X_dec_sym, mask_dec, y_sym, delta_inds_sym], loss_val_T, allow_input_downcast=True)
    else:
    ...
    if params['attention']:  
        if params['att_type']=='lambda_mu' or params['att_type']=='lambda_mu_alt':
            forecast = theano.function([X_enc_sym, mask_enc, X_enc_sym2, mask_enc2, X_enc_sym3, mask_enc3, X_enc_sym4, mask_enc4, X_dec_sym, mask_dec, delta_inds_sym], [network_output_val, alpha_weights], allow_input_downcast=True)
        else:
...
def network_mvrt4_text(params):
    ...
    mask_enc = T.matrix('enc_mask')
    mask_dec = T.matrix('dec_mask')
    y_sym = T.matrix('y_sym') #y_sym = T.fTensor3('y_sym') 
    X_enc_sym2 = T.ftensor3('x_enc_sym2')
    X_enc_sym3 = T.ftensor3('x_enc_sym3')
    X_enc_sym4 = T.ftensor3('x_enc_sym4')
    mask_enc2 = T.matrix('enc_mask2')
    ...
    if params['attention']:
        ...
        if params['num_layers'] ==1:
            #l_out, l_out_val= model_seq2seq_mvrt4(X_enc_sym, mask_enc, X_enc_sym2, mask_enc2, X_enc_sym3, mask_enc3, X_enc_sym4, mask_enc4, X_dec_sym, mask_dec, params['windowsize'], params['horizon'],  hidden_size = params['num_units'], 
              #                                  grad_clip = params['grad_clipping'], params['word_dimension']) 
            l_out, l_out_val= model_seq2seq_mvrt4(X_enc_sym, mask_enc, X_enc_sym2, mask_enc2, X_enc_sym3, mask_enc3, X_enc_sym4, mask_enc4, X_dec_sym, mask_dec, params['windowsize'], params['horizon'],  hidden_size = params['num_units'], 
                                                #grad_clip = params['grad_clipping'])
        network_output, network_output_val = lasagne.layers.get_output([l_out, l_out_val])
        #
    ...
    if params['regularization_type'] == 'l1':
        ...
        reg_loss = lasagne.regularization.regularize_network_params(l_out, l2) * params['lambda_regularization']
    loss_T = T.mean(lasagne.objectives.squared_error(network_output, y_sym)) + reg_loss #loss_T = T.nnet.categorical_crossentropy(network_output, y_sym).mean() + reg_loss
    loss_val_T = T.mean(lasagne.objectives.squared_error(network_output_val, y_sym)) # loss_val_T = T.nnet.categorical_crossentropy(network_output_val, y_sym) 
    if params['alg'] == 'adam':
        updates = lasagne.updates.adam(loss_T, weights, learning_rate=eta)
    else:
        logging.error("Optimization algorithm needs to be specified, e.g. 'adam'")
...
def train_mvrt4(params, results_directory):
    ...
    while (epoch < params['max_num_epochs']) and (not done_looping):
        ...
        if params['attention'] and params['att_type']=='lambda_mu_alt':
            d_inds = delta_gap_depth_mask(X_val_mask)#TODO: interpolation
            val_loss = f_val(X_val, X_val_mask, X_val2, X_val_mask2, X_val3, X_val_mask3, X_val4, X_val_mask4, X_val_dec, X_val_mask_dec, y_val, d_inds)
        else:
    ...
    if params['attention']:
        if params['att_type']=='lambda_mu':
            ...
            if params['interpolation']:
                ...
                y_pred, y_weights = forecast(X_test, X_test_mask, X_test2, X_test_mask2, X_test3, X_test_mask3, X_test4, X_test_mask4, X_dec_test, X_test_mask_dec, d_inds_test)
            else:
                y_pred, y_weights = forecast(X_test, X_test_mask, X_test2, X_test_mask2, X_test3, X_test_mask3, X_test4, X_test_mask4, X_dec_test, X_test_mask_dec, d_inds_test)
        elif params['att_type']=='lambda_mu_alt':
            d_inds = delta_gap_depth_mask(X_mask)
            if params['interpolation']:
                y_pred, y_weights = forecast(X_test, np.ones_like(X_test_mask), X_test2, np.ones_like(X_test_mask2), X_test3, np.ones_like(X_test_mask3), X_test4, np.ones_like(X_test_mask4), X_dec_test, X_test_mask_dec, d_inds_test)
            else:
                y_pred, y_weights = forecast(X_test, X_test_mask, X_test2, X_test_mask2, X_test3, X_test_mask3, X_test4, X_test_mask4, X_dec_test, X_test_mask_dec, d_inds_test)
...
def main():
    ...
    results_directory = 'Results_multivariate/'
    #    
    train_and_test = True
    if len(sys.argv) > 5:
    ...
    for i in line_inds:
        ...
        #
        if train_and_test:
            #if sys.argv[1] == 'fuzzy.params.text':
                #results_directory = 'Results_multivariate/coservit' 
            train_loss, validation_loss = train_mvrt4(params, results_directory)
        #
        test_mvrt4(params, results_directory)
        #        
        f = open(str(sys.argv[1])+"_mvrt.log","a")
        f.write(str(i)+':')
        #f.write(line)
        f.close()
